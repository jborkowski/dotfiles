...
description: improve an AGENTS.md file using <important if> blocks to improve instruction adherence
---

When the user provides an AGENTS.md file (or asks you to improve one), rewrite it following the principles and structure below.

AGENTS.md is the cross-tool convention (see https://agents.md) read by Claude Code, Cursor, Aider, and other coding agents. The same "may or may not be relevant" injection problem affects all of them — the `<important if>` pattern is a Claude-native relevance signal but harmless noise for other tools, so the file stays portable.

## Core Problem

Coding agents inject AGENTS.md into their system context with framing along the lines of:
...
This means the agent will ignore parts of your AGENTS.md it deems irrelevant. The more content that isn't applicable to the current task, the more likely the agent is to ignore everything — including the parts that matter.

## Solution: `<important if="condition">` Blocks

Wrap conditionally-relevant sections of the AGENTS.md in `<important if="condition">` XML tags. This exploits the same XML tag pattern used in Claude Code's own system prompt, giving the model an explicit relevance signal that cuts through the "may or may not be relevant" framing. Other agents treat the tags as inert markup, so portability is preserved.
...
- Frontier models can reliably follow a few hundred instructions. Coding agents' system prompts and tools already use ~50 of those. Your AGENTS.md should be as lean as possible.
...
When rewriting an AGENTS.md, produce this structure:
...
When given an existing AGENTS.md to improve:
